...
|-------------------------------------------------------------------|----------------------------------|--------------------|
| Управляет ACL для одного пользователя и одного топика             | `acl-user-topic.sh`        | `./acl-user-topic.sh user-alice test-1 add`                  |
| Управляет ACL для одного пользователя сразу для нескольких топиков | `acl-user-multi-topics.sh`| `./acl-user-topics.sh user-alice "test-1 test-2 test-3" add` |
---

## Параметры

Все скрипты используют четыре основных аргумента:

1. **USER** – имя пользователя (Kafka principal)
2. **TOPIC** или **TOPICS** – топик или список топиков через пробел
3. **ACTION** – `add` или `remove`
4. **OPERATION** - `Read`, `Write` или `All`
...
3. Перезагрузка брокера не требуется после добавления или удаления ACL.
